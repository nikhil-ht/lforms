...
/* WEBPACK VAR INJECTION */(function(global) {/* harmony import */ var _index__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(3);
/* harmony import */ var _finally__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(7);
/* harmony import */ var _allSettled__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(8);
...
var LForms = __webpack_require__(11);
...
LForms.Util.FHIRSupport = __webpack_require__(29);

__webpack_require__(30);

LForms._elementResizeDetectorMaker = __webpack_require__(31);
module.exports = LForms;
...
(function () {
  ...
  angular.module('lformsWidget', widgetDeps).config(['$animateProvider', '$rootScopeProvider', '$compileProvider', function ($animateProvider, $rootScopeProvider, $compileProvider) {
    ...
    $rootScopeProvider.digestTtl(20); // Allow "data:" URLs to be assigned to an href.
...
var LForms = Object.assign({}, __webpack_require__(14));
module.exports = LForms;
...
module.exports = JSON.parse("{\"lformsVersion\":\"29.3.1\"}");
...
angular.module('lformsWidget').controller('LFormsCtrl', ['$window', '$scope', '$element', '$timeout', '$interval', '$sce', 'smoothScroll', 'LF_CONSTANTS', 'lformsConfig', function ($window, $scope, $element, $timeout, $interval, $sce, smoothScroll, LF_CONSTANTS, lformsConfig) {
  ...
  */
  ...
  $scope.createCustomHeaders = function (item) {
    ...
    item.items.forEach(function (questionItems) {
      questionItems.items.forEach(function (subItems) {
        var _subItems$skipLogic, _subItems$skipLogic$c, _subItems$skipLogic$c2;

        if (!_this.uniqueGridHeaders.includes(subItems.question) && ((_subItems$skipLogic = subItems.skipLogic) === null || _subItems$skipLogic === void 0 ? void 0 : (_subItems$skipLogic$c = _subItems$skipLogic.conditions[0]) === null || _subItems$skipLogic$c === void 0 ? void 0 : (_subItems$skipLogic$c2 = _subItems$skipLogic$c.trigger) === null || _subItems$skipLogic$c2 === void 0 ? void 0 : _subItems$skipLogic$c2.value) != 'alwaysHide') {
          _this.uniqueGridHeaders.push(subItems.question);
        }
      });
    });
  };
...
(function () {
  ...
  var LForms = __webpack_require__(12); //  var angular = require('angular');
      ...
      controller: ['$scope', function ($scope) {
        ...
        $scope.createAttachment = function (item) {
          if (!item._fileInfo && !item._attachmentURL) {
            ...
            item.value = {
              title: item._attachmentName || ((_item$_fileInfo = item._fileInfo) === null || _item$_fileInfo === void 0 ? void 0 : _item$_fileInfo.name)
            };
            var value = item.value;
            if (item._attachmentURL) value.url = item._attachmentURL;

            if (item._fileInfo) {
              // attach the data too
              var fileInfo = item._fileInfo;
              value.contentType = fileInfo.type;
              if (fileInfo.lastModified) value.creation = new Date(fileInfo.lastModified).toISOString();else if (fileInfo.lastModifiedDate) // IE 11
              ...
              reader.onload = function (loadEvent) {
                ...
                if (data.indexOf('data:') != 0 || commaIndex < 0) {
                  throw new Error('data URL did not start with expected prefix, but with ' + data.slice(0, 30));
        ...
        element.bind("change", function (changeEvent) {
          if (changeEvent.target.files) {
            ...
            if (!newFile.type) {
              ...
              // type.
              alert('Unknown file type.  Please ensure the file has an ' + 'appropriate extension');
              newFile = null; // don't proceed
            } else if (item.allowedAttachmentTypes && item.allowedAttachmentTypes.indexOf(newFile.type) < 0) {
              var types = item.allowedAttachmentTypes;
              ...
              var msg = 'Adding a large file as an attachment might cause your ' + 'computer to run low on memory.  There is a button to enter a ' + 'URL instead of attaching the file data.  Are you sure you want ' + 'to attach the file data?';
 ...
 */
var moment = __webpack_require__(22); // Acceptable date formats
// Strict parsing -
...
LForms.Util = {
  ...
  addFormToPage: function addFormToPage(formDataDef, formContainer, options) {
    ...
    var rtnPromise = new Promise(function (resolve, reject) {
      ...
      formContainer.html('<div id=lformsErrors style="color: red"><b>Unable to ' + 'display the form, due to the following error:</b><p id=lformsErrorContent></p>');
  ...
  getServerFHIRReleaseID: function getServerFHIRReleaseID(callback) {
    ...
    if (!LForms._serverFHIRReleaseID) {
      ...
      try {
        var fhirAPI = LForms.fhirContext; //fhirAPI.request('metadata?_elements=fhirVersion').then(function(res) // causes an error on lforms-smart-fhir (TBD)
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _fhir_fhir_common__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(25);
/**
...
(function () {
  ...
  LForms.LFormsData = Class({
     ...
     */
    _buildURL: function _buildURL(pathParts, queryParams) {
      ...
      var url = pathParts.map(function (part) {
        return encodeURIComponent(part);
      }).join('/');

      if (queryParams) {
        // Assuming a single value per key, for now.
        url += '?' + Object.keys(queryParams).map(function (k) {
    ...
    _findValueSetIDAndSearch: function _findValueSetIDAndSearch(item, fieldVal, count) {
      ...
      var fhirClient = LForms.fhirContext; // Cache the lookup of ValueSet IDs, which should not change.  (A page
...
var stateHandler = __webpack_require__(41); //Detection strategies.
